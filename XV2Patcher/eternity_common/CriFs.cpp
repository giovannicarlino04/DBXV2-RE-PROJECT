...
uint8_t *CriFs::ReadFile(const std::string &path, size_t *psize)
{
    ...
    {
        uint64_t size;
        buf = cpk->ExtractFile(conv_path, &size);
...
bool CriFs::LoadFile(BaseFile *file, const std::string &path)
{
    size_t size;
    uint8_t *buf = ReadFile(path, &size);
...
bool CriFs::FileExists(const std::string &file)
{
