...
TiXmlElement *BcsColor::Decompile(TiXmlNode *root, int idx) const
{
...
TiXmlElement *BcsPartColors::Decompile(TiXmlNode *root, int idx) const
{
    ...
    if (valid)
    {
        ...
        {
            colors[i].Decompile(entry_root, (int)i);
        }
    }
    else
    {
...
bool BcsFile::Load(const uint8_t *buf, size_t size)
{
    ...
    if (hdr->skeleton2_table_offset != 0)
    {
        ...
        if (!skeleton_data2.Load(this, buf, (const BCSSkeletonData *)GetOffsetPtr(hdr, skeleton_table, 0)))
            return false;
    }
...
uint8_t *BcsFile::Save(size_t *psize)
{
    ...
    sk2_size = skeleton_data2.PreSave(&str_size,  strings);
    ...
    if (skeleton_data2.valid)
    {
        skl2_table[0] = current_offset;
        current_offset += (uint32_t) skeleton_data2.Save(this, buf, (BCSSkeletonData *)(buf+current_offset), strings_offs);
    }
...
TiXmlDocument *BcsFile::Decompile() const
{
    ...
    TiXmlElement *skl2_root = skeleton_data2.Decompile(root);

    skl_root->SetAttribute("idx", 0);
    skl2_root->SetAttribute("idx", 1);
...
bool BcsFile::Compile(TiXmlDocument *doc, bool)
{
    ...
    for (const TiXmlElement *elem = root->FirstChildElement(); elem; elem = elem->NextSiblingElement())
    {
        ...
        else if (name == "SkeletonData")
        {
            ...
            {
                if (!skeleton_data2.Compile(elem))
                    return false;
            }
