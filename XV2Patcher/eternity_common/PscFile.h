#ifndef __PSCFILE_H__
#define __PSCFILE_H__
...
STATIC_ASSERT_STRUCT(PSCHeader, 0x10);
...
public:
    ...
    PscSpecEntry *FindSpecFromAbsolutePos(size_t pos);
    PscEntry *FindEntry(uint32_t char_id);

    PscSpecEntry *FindSpec(uint32_t char_id, uint32_t costume_id);

    inline void AddEntry(const PscEntry &entry) { entries.push_back(entry); }
    void RemoveEntry(uint32_t char_id);

    const std::vector<PscEntry> &GetEntries() const { return entries; }
    std::vector<PscEntry> &GetEntries() { return entries; }
