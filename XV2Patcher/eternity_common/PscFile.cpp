...
bool PscFile::Load(const uint8_t *buf, size_t size)
{
    ...
    const PSCEntry *file_entries = (const PSCEntry *)(hdr+1);
    const PSCSpecEntry *file_spec = (const PSCSpecEntry *)(file_entries+hdr->num_entries);
    ...
    for (size_t i = 0; i < entries.size(); i++)
    {
        ...
        entry.specs.resize(file_entries[i].num_specs);
...
PscSpecEntry *PscFile::FindSpecFromAbsolutePos(size_t pos)
{
...
PscSpecEntry *PscFile::FindSpec(uint32_t char_id, uint32_t costume_id)
{
    PscEntry *entry = FindEntry(char_id);
    if (!entry)
        return nullptr;
